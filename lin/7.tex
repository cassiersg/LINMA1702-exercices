...
\begin{enumerate}
  ...
  \item Vous disposez de 4 objets de poids respectifs 5, 3, 8 et 7. Les utilités des
    ...
    \begin{solution}
      ...
      Malheureusement, la solution optimale du problème relaxé est $(1,1,0.5,0)$, qui n'est pas entière.
      ...
      En essayant toutes les autres combinaisons,
      on voit qu'il s'agit de la solution optimale en nombres entiers.
      La solution du problème non-relaxé est donc $(0,1,1,0)$.
      C'est un avantage des problèmes en nombre entier, on peut parfois énumérer toutes les solutions admissibles.

      On peut aussi le faire de façon plus sytématique avec l'algorithme ``branch and bound''

      \Tree [.{$(1,1,0.5,0) \rightarrow 39.5$}
        [.{$x_3 \leq 0$\\$(1,1,0,4/7) \rightarrow 31$} ]
        [.{$x_3 \geq 1$\\$(0,1,1,0) \rightarrow 35$} ]
      ]

      De la branche avec $x_3 \geq 1$, on tire $z^* \geq 35$.
      La branche avec $x_3 \leq 0$ ne fera pas mieux que 31,
      il est donc inutile de continuer dans cette direction.
      On a donc la solution optimale $(0,1,1,0)$ avec un
      coût optimal de 35.
    \end{solution}
